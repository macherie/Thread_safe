...
bool thread_post(thread_t *thread, thread_fn func, void *context) {
  ...
  if(!thread || !func) {
     printf("%s: thread or func is NULL", __func__);
     return false;
  }
  ...
  if (!item) {
    printf("%s unable to allocate memory: %s", __func__, strerror(errno));
    return false;
  }
  item->func = func;
...
void thread_stop(thread_t *thread) {
  assert(thread != NULL);
  if(!thread) {
     printf("%s: thread is NULL", __func__);
     return ;
  }
...
static void *run_thread(void *start_arg) {
  ...
  if (prctl(PR_SET_NAME, (unsigned long)thread->name) == -1) {
    printf("%s unable to set thread name: %s", __func__, strerror(errno));
    start->error = errno;
  ...
  if (count > WORK_QUEUE_CAPACITY)
    printf("%s growing event queue on shutdown.", __func__);
...
static void work_queue_read_cb(void *context) {
  assert(context != NULL);
  if(!context) {
     printf("%s: thread context is NULL", __func__);
     return;
  }
