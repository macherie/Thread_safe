...
config_t *config_new(const char *filename) {
  ...
  if (!config) {
    printf("%s unable to allocate memory for config_t.", __func__);
    fclose(fp);
    return NULL;
  }
...
static void config_parse(FILE *fp, config_t *config) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (*line_ptr == '[') {
      ...
      if (line_ptr[len - 1] != ']') {
        printf("%s unterminated section name on line %d.", __func__, line_num);
        continue;
      }
      ...
      if (!split) {
        printf("%s no key/value separator found on line %d.", __func__, line_num);
        continue;
      }
